...
bool LaserMapping::LoadParams(ros::NodeHandle &nh) {
    // Step:1.设定特征提取器相关参数
    // KeyPoints params
    auto InitKeypointsExtractor = [&]()
    {
        #define SetKeypointsExtractorParam(type, rosParam, keParam) {type val; if (nh.getParam(rosParam, val)) ke->Set##keParam(val);}
        SetKeypointsExtractorParam(int,   "n_threads", NbThreads)
        SetKeypointsExtractorParam(int,   "neighbors_side_nb", MinNeighNb)
        SetKeypointsExtractorParam(float,  "neighbors_radius", MinNeighRadius)
        SetKeypointsExtractorParam(float, "min_distance_to_sensor", MinDistanceToSensor)
        SetKeypointsExtractorParam(float, "max_distance_to_sensor", MaxDistanceToSensor)
        SetKeypointsExtractorParam(float,  "min_beam_surface_angle", MinBeamSurfaceAngle)
        SetKeypointsExtractorParam(float, "min_azimuth", AzimuthMin)
        SetKeypointsExtractorParam(float,  "max_azimuth", AzimuthMax)
        SetKeypointsExtractorParam(float,  "plane_sin_angle_threshold", PlaneSinAngleThreshold)
        SetKeypointsExtractorParam(float,  "edge_sin_angle_threshold", EdgeSinAngleThreshold)
        SetKeypointsExtractorParam(float,  "edge_depth_gap_threshold", EdgeDepthGapThreshold)
        SetKeypointsExtractorParam(float,  "edge_nb_gap_points", EdgeNbGapPoints)
        SetKeypointsExtractorParam(float,  "edge_intensity_gap_threshold", EdgeIntensityGapThreshold)
        SetKeypointsExtractorParam(int,    "max_points", MaxPoints)
        SetKeypointsExtractorParam(float,  "voxel_grid_resolution", VoxelResolution)
        SetKeypointsExtractorParam(float,  "input_sampling_ratio", InputSamplingRatio)
        #define EnableKeypoint(kType) \
        { \
        bool enabled = false; \
        std::string name = LidarSlam::KeypointTypeNames.at(kType); \
        if (nh.getParam(name, enabled)) \
            UseKeypoints[kType] = enabled; \
        if (enabled) \
    ...
    {
        if (UseKeypoints[k])
        UsableKeypoints.push_back(k);
    }

    ke->Enable(UsableKeypoints);
    ...
    nh.param<bool>("feature_extract_enable", feature_extract, false);
...
void LaserMapping::PublishKeyPointsWorld(CloudS::Ptr &key_points, ros::Publisher& topic) {
    ...
    pcl::toROSMsg(*laserCloudEdgeWorld, laserCloudEdgemsg);
    // ?:这个时间戳不确定
    laserCloudEdgemsg.header.stamp = ros::Time().fromSec(lidar_end_time_);
    laserCloudEdgemsg.header.frame_id = "camera_init";
...
void LaserMapping::Run() {
    ...
    if(feature_extract){
        ...
        for (int i = 0; i < scan_undistort_->size(); i++) {
            ...
            //?:这里有个问题是，点云的时间偏移是否还需要转化为秒，特征提取部分是否还需用到时间
            lidar_point.time = point.curvature * 1e-6;    //unit:s
            lidar_point.intensity = point.intensity;
            lidar_point.laser_id = point.ring;
        ...
        // Step:2点云特征提取
        ke->ComputeKeyPoints(cloudS_ptr);
        for (auto k : UsableKeypoints)
            CurrentRawKeypoints[k] = ke->GetKeypoints(k);

        // Step:3特征点可视化
        std::cout << "Extracted features : ";
        for (auto k : UsableKeypoints)
            std::cout << CurrentRawKeypoints[k]->size() << " " << KeypointTypeNames.at(k) << " ";
        std::cout << std::endl;
    ...
    if(feature_extract){
        ...
        for (auto k : UsableKeypoints)
        {
            CurrentRawKeypoints[k].reset(new CloudS);
        }
...
void LaserMapping::PointBodyToWorld(const PointS *pi, PointType *const po) {
    common::V3D p_body(pi->x, pi->y, pi->z);
    common::V3D p_global(state_point_.rot * (state_point_.offset_R_L_I * p_body + state_point_.offset_T_L_I) +
                         state_point_.pos);
