...
class LaserMapping {
   ...
   private:
    ...
    void PointBodyToWorld(PointS const *pi, PointType *const po);
   ...
   private:
    ...
    std::map<LidarSlam::Keypoint, bool> UseKeypoints = {{LidarSlam::EDGE, true}, {LidarSlam::INTENSITY_EDGE, true}, {LidarSlam::PLANE, true}, {LidarSlam::BLOB, false}};
    std::vector<LidarSlam::Keypoint> UsableKeypoints = {};
    // 用来存储当前帧特征点
    std::map<LidarSlam::Keypoint, CloudS::Ptr> CurrentRawKeypoints;
    ...
    std::shared_ptr<IVoxType> ivox_corner = nullptr;
