...
struct EIGEN_ALIGN16 Point {
    PCL_ADD_POINT4D;
    float intensity;
    float curvature;   //used as time
    std::uint16_t ring;
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW
};

struct EIGEN_ALIGN16 Point_Ori {
    PCL_ADD_POINT4D;
    float intensity;
    float time;
    std::uint16_t ring;
...
POINT_CLOUD_REGISTER_POINT_STRUCT(velodyne_ros::Point,
                                  (float, x, x)(float, y, y)(float, z, z)(float, intensity, intensity)
                                      (float, curvature, curvature)(std::uint16_t, ring, ring))

POINT_CLOUD_REGISTER_POINT_STRUCT(velodyne_ros::Point_Ori,
                                  (float, x, x)(float, y, y)(float, z, z)(float, intensity, intensity)
